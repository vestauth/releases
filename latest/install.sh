...
#  |  curl -sfS "https://vestauth.sh?version=1.41.0" | sh                                            |
#  |  curl -sfS "https://vestauth.sh?directory=." | sh                                               |
#  |  curl -sfS "https://vestauth.sh?directory=/custom/path&version=1.41.0" | sh                     |
#  |                                                                                                 |
...
usage() {
  ...
  echo "  --directory       directory to install vestauth to (default: \"/usr/local/bin\")"
  echo "  --force           force reinstallation even if already installed (default: false)"
  echo "  --version         version of vestauth to install (default: \"$VERSION\")"
  echo ""
  echo "Commands:"
  echo "  install           install vestauth"
...
is_os_supported() {
  ...
  *)
    ...
    echo "? request support by opening an issue at [https://github.com/vestauth/vestauth/issues]"
...
is_installed() {
  ...
  echo "[vestauth@$current_version] already installed ($(directory)/$(binary_name))"
...
filename() {
  echo "vestauth-$VERSION-$(os_arch).tar.gz"

  return 0
}

download_url() {
  echo "$REGISTRY_URL/@vestauth/vestauth-$(os_arch)/-/vestauth-$(os_arch)-$VERSION.tgz"
...
which_vestauth() {
  local result
  result=$(command -v vestauth 2>/dev/null) # capture the output without displaying it on the screen
...
warn_of_any_conflict() {
  local vestauth_path="$(which_vestauth)"

  if [ "$vestauth_path" != "" ] && [ "$vestauth_path" != "$(directory)/$(binary_name)" ]; then
    echo "[VESTAUTH_CONFLICT] conflicting vestauth found at $vestauth_path" >&2
...
install_vestauth() {
  ...
  # TODO: handle vestauth.exe when on a windows machine? binary is not package/vestauth, it's package/vestauth.exe
  ...
  # let user know
  echo "[vestauth@$VERSION] installed successfully ($(directory)/$(binary_name))"
  echo "now type: vestauth help"
...
run() {
  ...
  if [ -n "$VERSION" ]; then
    ...
    if is_installed "$VERSION"; then
      echo "[vestauth@$VERSION] already installed ($(directory)/$(binary_name))"

      return 0
    else
      install_vestauth "$VERSION"
    fi
  else
